...
public class pg9a1{
	...
	/** runs the checkLine() method 5 times recursively
	 *	@param x the line number it is at
	 * @return string of text that contains everything the assignment called for
	 */
	public static String pg(int x){
		...
		if(x>0){	
			System.out.println("Enter line " + x + " of text to check:");
			orig = keyboard.nextLine();
			line = orig.toLowerCase().replaceAll(" ", "");
			//System.out.println(line);
			
			if(checkLine(line))
				return "Line " + x + " '" + orig + "' is: Valid\n" + pg(x+1);
			else
				return"Line " + x + " '" + orig + "' is: Invalid\n" + pg(x+1);
		}
		return "";
	}//end pg
